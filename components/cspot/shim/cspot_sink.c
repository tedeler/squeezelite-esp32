...
extern void *cspot_create(const char *name, cspot_cmd_cb_t cmd_cb, cspot_data_cb_t data_cb);
...
// for now there is no feedback
#define cspot_cmd(ctx, event, param)
...
static bool cmd_handler(cspot_event_t event, ...) {
	...
	switch(event) {
	case CSPOT_SINK_SETUP:
		actrls_set(controls, false, NULL, actrls_ir_action);
		displayer_control(DISPLAYER_ACTIVATE, "CSpot (Spotify)");
		break;
	...
	case CSPOT_SINK_STOP:
		...
		break;
	case CSPOT_SINK_METADATA: {
		char *artist = va_arg(args, char*), *album = va_arg(args, char*), *title = va_arg(args, char*);
		displayer_metadata(artist, album, title);
		break;
	}	
	case CSPOT_SINK_PROGRESS: {
